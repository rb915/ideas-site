...
def main():
    ...
    def is_origin_stories(row):
        return row["source"].strip().lower() == "origin stories"

    def is_whitespace(row):
        return row["source"].strip().lower() == "whitespace"

    def is_5k_products(row):
        return "5k" in row["source"].strip().lower()
...
{bn_items_joined}
    ...
    # ---- Build the Origin Stories section HTML ----
    origin_stories_rows = [r for r in rows if is_origin_stories(r)]
    origin_section_html = ""
    if origin_stories_rows:
        os_items_html = []
        for r in origin_stories_rows:
            idea_id = html.escape(r["id"])
            title = html.escape(r["title"].strip())
            slot = html.escape(r["slot"].strip())
            source = html.escape(r["source"].strip())
            ...
            if created: meta_bits.append(f'<span class="tag tag-date">{created}</span>')
            meta_html = ('<div class="item-meta">' + "".join(meta_bits) + "</div>") if meta_bits else ""
            os_items_html.append(f'''    <article class="idea" data-id="{idea_id}">
      <details class="idea-details">
        <summary class="idea-summary">
          <span class="idea-title">{title}</span>
          <button class="dismiss-btn" onclick="dismissIdea(event, '{idea_id}')" title="Dismiss idea">✕</button>
...
{os_items_joined}
    ...
    # ---- Build the Whitespace Alerts section HTML ----
    whitespace_rows = [r for r in rows if is_whitespace(r)]
    ...
    if whitespace_rows:
        ...
        for r in whitespace_rows:
            ...
            if created: meta_bits.append(f'<span class="tag tag-date">{created}</span>')
            meta_html = ('<div class="item-meta">' + "".join(meta_bits) + "</div>") if meta_bits else ""
            ws_items_html.append(f'''    <article class="idea" data-id="{idea_id}">
      <details class="idea-details">
        <summary class="idea-summary">
          <span class="idea-title">{title}</span>
          <button class="dismiss-btn" onclick="dismissIdea(event, '{idea_id}')" title="Dismiss idea">✕</button>
...
{content_html}
        ...
        ws_items_joined = "\n".join(ws_items_html)
        whitespace_section_html = f'''
<details class="format-section">
  <summary>
    <span class="section-num">\u25cb</span>
    <span class="section-title">Whitespace Alerts</span>
    <span class="section-count">{len(whitespace_rows)}</span>
    <svg class="chevron" viewBox="0 0 12 12" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M4 2 L8 6 L4 10"/></svg>
  </summary>
  <div class="items">
{ws_items_joined}
    ...
    # ---- Build the Products to Start with $5K section HTML ----
    fivek_rows = [r for r in rows if is_5k_products(r)]
    fivek_section_html = ""
    if fivek_rows:
        fk_items_html = []
        for r in fivek_rows:
            ...
            if created: meta_bits.append(f'<span class="tag tag-date">{created}</span>')
            meta_html = ('<div class="item-meta">' + "".join(meta_bits) + "</div>") if meta_bits else ""
            fk_items_html.append(f'''    <article class="idea" data-id="{idea_id}">
      <details class="idea-details">
        <summary class="idea-summary">
          <span class="idea-title">{title}</span>
          <button class="dismiss-btn" onclick="dismissIdea(event, '{idea_id}')" title="Dismiss idea">✕</button>
...
{content_html}
        ...
        fk_items_joined = "\n".join(fk_items_html)
        fivek_section_html = f'''
<details class="format-section">
  <summary>
    <span class="section-num">$</span>
    <span class="section-title">Products to Start with $5K</span>
    <span class="section-count">{len(fivek_rows)}</span>
    <svg class="chevron" viewBox="0 0 12 12" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M4 2 L8 6 L4 10"/></svg>
  </summary>
  <div class="items">
{fk_items_joined}
    ...
    # ---- Build the Roast section HTML ----
    roast_rows = [r for r in rows if is_roast(r)]
    roast_section_html = ""
    if roast_rows:
        ro_items_html = []
        for r in roast_rows:
            ...
            if created: meta_bits.append(f'<span class="tag tag-date">{created}</span>')
            meta_html = ('<div class="item-meta">' + "".join(meta_bits) + "</div>") if meta_bits else ""
            ro_items_html.append(f'''    <article class="idea" data-id="{idea_id}">
      <details class="idea-details">
        <summary class="idea-summary">
          <span class="idea-title">{title}</span>
          <button class="dismiss-btn" onclick="dismissIdea(event, '{idea_id}')" title="Dismiss idea">✕</button>
...
{content_html}
        ...
        ro_items_joined = "\n".join(ro_items_html)
        roast_section_html = f'''
<details class="format-section">
  <summary>
    <span class="section-num">\U0001f525</span>
    <span class="section-title">Roast</span>
    <span class="section-count">{len(roast_rows)}</span>
    <svg class="chevron" viewBox="0 0 12 12" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M4 2 L8 6 L4 10"/></svg>
  </summary>
  <div class="items">
{ro_items_joined}
